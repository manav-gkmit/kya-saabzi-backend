"""Background dish enrichment via LLM metadata lookup."""
...
async def _copy_metadata_if_exists(
    ...
    if existing_enriched:
        ...
        if existing_enriched.ingredients and not dish.ingredients:
            await attach_ingredients_to_dish(
...
async def _apply_gemini_result(
    ...
    for d in dishes_to_update:
        if not d.ingredients and result.ingredients:
            await attach_ingredients_to_dish(db, d, result.ingredients)
...
async def enrich_dish_background_task(dish_id: UUID) -> None:
    """Background task — fully async non-blocking enrichment."""
    async with SessionLocal() as db:
        try:
            ...
            if await _copy_metadata_if_exists(db, dish, dish_name_lower):
                return
            ...
            try:
                ...
                result = await enrich_dish_with_gemini(dish.name)
                await _apply_gemini_result(db, dish.name, dish_name_lower, result)
            finally:
                async with _ENRICHMENT_LOCK:
                    _ENRICHMENT_IN_PROGRESS.discard(dish_name_lower)

        except Exception:
            logger.exception("Error in enrich_dish_background_task")
            await db.rollback()
