"""Dish search & creation — core business logic."""
...
async def get_exact_dish(db: AsyncSession, input_name: str, household_id: UUID) -> Dish | None:
    stmt = (
        select(Dish)
        .where(
...
async def get_fuzzy_dish(db: AsyncSession, input_name: str, household_id: UUID) -> Dish | None:
    first_word = (
...
async def find_or_create_dish(
    ...
    *,
    ...
    if dish:
        ...
        if ingredients:
            await attach_ingredients_to_dish(db, dish, ingredients)
            await db.flush()
        return dish

    dish = await get_fuzzy_dish(db, input_name, household_id)
    if dish:
        if ingredients:
            await attach_ingredients_to_dish(db, dish, ingredients)
            await db.flush()
